...
DOCKER_IMAGE_TAG="example_algorithm_challenge-only-track-1-sanity-check-submission-phase"

DOCKER_NOOP_VOLUME="${DOCKER_IMAGE_TAG}-volume"

INPUT_DIR="${SCRIPT_DIR}/test/input"
OUTPUT_DIR="${SCRIPT_DIR}/test/output"
INTERFACE_DIRS=("case0" "case1" "case2")
...
for interface_dir in "${INTERFACE_DIRS[@]}"; do
  if [ -d "${OUTPUT_DIR}/${interface_dir}" ]; then
    # This allows for the Docker user to write
    chmod -f o+rwX "${OUTPUT_DIR}/${interface_dir}"

    echo "=+= Cleaning up earlier output for ${interface_dir}"
    ...
    mkdir -p -m o+rwX "${OUTPUT_DIR}/${interface_dir}"
  fi
done
...
for interface_dir in "${INTERFACE_DIRS[@]}"; do
  run_docker_forward_pass "$interface_dir"
done
